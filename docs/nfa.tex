...
\begin{tikzpicture} [
    ...
    \node[state]            (3)  [right of = 1]         {$WtMnem$};
    \node[state]            (4)  [right of = 3]         {$MnPre$};
    \node[state]            (5)  [right of = 4]         {$Mnem$};
    \path (1) -- node[coordinate] (mid13) {} (3);
    \node[state]            (2)  [below of=mid13, node distance=3.464cm]    {$Label$};

    \node[state]            (6)  [below of = 3, node distance=6cm]          {$WtOp1$};
    ...
    \node[state]            (9)  [below of = 6, node distance=6cm]          {$WtOp2$};
    \node[state]            (10) [right of = 9]         {$Op2$};
    \node[state]            (12) [right of = 10]        {$WtDone$};

    \node[state, accepting] (11) [below right of = 5]   {$Done$};

    \path
    (1) % Start
    edge    [loop below]                                    node {blank}    (1)
    edge    [sloped, anchor=center, above]                  node {alpha}    (2)
    edge    [sloped, anchor=center, above]                  node {$\varepsilon$}    (3)
    edge    [sloped, anchor=center, above, out=30, in=90]   node {END}      (11)

    (2) % Label
    edge    [loop below]                                    node {alnum}    (2)
    edge    [sloped, anchor=center, above]                  node {blank}    (3)

    (3) % WtMnem
    edge    [loop below]                                    node {blank}    (3)
    edge    [sloped, anchor=center, above]                  node {+}        (4)
    edge    [sloped, anchor=center, above, out=30, in=150]  node {alpha}    (5)
    ...
    edge    [sloped, anchor=center, above, out=270, in=90]  node {blank}    (6)
    edge    [sloped, anchor=center, above, out=315, in=135] node {END}      (11)

    (6) % WtOp1
    edge    [loop below]                                    node {blank}    (6)
    edge    [sloped, anchor=center, above]                  node {alnum}    (7)
    edge    [sloped, anchor=center, above, out=300, in=120] node {@, \#}    (13)
    edge    [sloped, anchor=center, above, out=30, in=180]  node {END}      (11)

    (13) % Op1Pre
    edge    [sloped, anchor=center, above, out=60, in=240]  node {alnum}    (7)

    (7) % Op1
    edge    [loop below, align=center]                      node {alnum,\\ quote content}    (7)
    edge    [sloped, anchor=center, above]                  node {blank}    (8)
    edge    [sloped, anchor=center, above, out=300, in=30]  node {comma}    (9)
    edge    [sloped, anchor=center, above, out=30, in=195]  node {END}      (11)

    (8) % WtComma
    edge    [loop right]                                    node {blank}    (8)
    edge    [sloped, anchor=center, above, out=240, in=15]  node {comma}    (9)
    edge    [sloped, anchor=center, above, out=60, in=210]  node {END}      (11)
    ...
    edge    [sloped, anchor=center, above, out=30, in=300]  node {END}   (11)

    (12) % WtDone
    edge    [loop below]                                    node {blank}    (12)
    edge    [sloped, anchor=center, above, out=0, in=315]   node {END}      (11);
\end{tikzpicture}
